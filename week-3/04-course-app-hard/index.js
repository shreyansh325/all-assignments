...
mongoose.connect("mongodb+srv://shreyansh325:B5gRCXnLbyNCe0nm@cluster0.cqiajmm.mongodb.net/course");
...
app.post('/admin/login', async (req, res) => {
  ...
  const validUser = await Admin.findOne({'username': tmpUser.username, 'password': tmpUser.password});
  if(!validUser)
...
app.post('/users/login', async (req, res) => {
  const tmpUser = new User(req.headers);
  const validUser = await User.findOne({'username':tmpUser.username, 'password': tmpUser.password});
  if(!validUser)
